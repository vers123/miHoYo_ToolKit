...
class UserPostsPage(QWidget):
    ...
    def _setup_ui(self):
        ...
        layout.addWidget(self._make_label("米游社用户", "pageTitle"))
        layout.addWidget(self._make_label("抓取用户发帖 HTML，提取发帖时间", "pageSubtitle"))
        ...
        self.btn_extract = QPushButton("提取时间")
    ...
    def _get_func(self, action: str, incremental: bool):
        if action == "fetch":
            ...
            return lambda: run_user(incremental=incremental)
        else:
            from extractors import run_extract_time
            return lambda: run_extract_time(incremental=incremental)
    ...
    def _run_task(self, action, incremental):
        ...
        self.progress.start(text=f"{mode}{action_text}用户发帖中...")
